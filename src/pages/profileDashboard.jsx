...
import React, { useState, useEffect } from "react";
import {User, Edit3, Calendar, MapPin, Phone, Mail, Clock, CheckCircle, AlertCircle, Loader2, Shield, Tag } from "lucide-react";
        ...
        <div className="lg:col-span-2">
          <div className="bg-white/90 p-8 rounded-3xl shadow-xl border border-white/20">
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="text-sm text-gray-500">{bookings.length} active bookings</div>
            </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                    <div className="flex-1">
                      ...
                      {booking.rooms.length > 0 ? (
                        booking.rooms.map((room) => (
                          <div key={room.id}>
                            <h4 className="text-lg font-semibold text-gray-600">
                              Room No:{' '}{room.room_number} - {room.room_type?.type}
                            </h4>
                          </div>
                        ))
                      ) : (
                        ...
                        <span
                          className={`inline-flex items-center gap-1 px-3 py-1 text-xs font-medium rounded-full border ${getStatusColor(booking.booking_status)}`}
                        >
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-bold text-blue-600">${booking.payment.total_payment}</div>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto p-8">
            <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Edit Profile</h3>
            <div className="space-y-6">
              <div>
